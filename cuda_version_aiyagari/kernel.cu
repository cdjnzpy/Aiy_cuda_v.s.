...
__global__ void back_utility( double* u_list) {
    //(7,300) * 300
    int i_s = blockIdx.x;//1-7shock
    int i_x_now_k = blockIdx.y;//this time k for 1-300 in N
    int i_x_next_k = threadIdx.x;//next time k for 1-300 in N
...
__global__ void back_max_value(double* V_input, double* u_list_output, double *V_output, int *policy_choice) {
    ...
    __syncthreads();
    if (i_s * Nk + i_k == 100000) {
        for (int check_k; check_k < Nk; check_k++) {
            //printf("check V_line %.5f , check utility %.5f \n", V_line[check_k], u_list_output[0 * Nk * Nk + 10 * Nk + check_k]);
            printf("check Value fun %.5f\n", V_output[0 * Nk + check_k]);
        }
        printf("***************************************************************************************************\n");
...
void main() {
    ...
    while (iter1 <= 50) {
        ...
        dim3 blocks_dim_setting(N, Nk);
        back_utility <<< blocks_dim_setting, Nk >>> (u_list_output);
